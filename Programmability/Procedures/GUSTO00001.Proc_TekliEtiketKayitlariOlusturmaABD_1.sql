...
GO
CREATE PROCEDURE [GUSTO00001].[Proc_TekliEtiketKayitlariOlusturmaABD_1] 
...
select ise.IsEmri_rowid,ise.ldPlanlananIsEmriMiktari [İş Emri Adedi],s.StokKodu_rowid [StokKodu_Rowid],s.sAd [Stok Adı],tpc.ldSayi [Pakete Girecek Olan Ürün Sayısı],1  [bTekliCokluPaket],
...
FETCH NEXT FROM TekliPaketBaslikBilgileri INTO @pIsemri,@ldIsEmriMiktari,@StokKodu_Rowid,@StokAdi,@ldPaketeGirecekAdet,@bTekliCokluPaket,@ldCokluEtiketSayisi,@EtiketAciklamasi,@StokKodu,@pCari,@sUreticiKodu,@ldTekliEtiketSayisi,@sSipRefereransNo
WHILE @@FETCH_STATUS =0

BEGIN

insert into [Gusto].[Bpm_IsEmriEtiketBaslikABD] ([pIsemri],[ldIsEmriMiktari],[pStokKodu],[sStokAdi],[ldPaketeGirecekMiktar],[bTekliCokluPaket],nBasilacakCokluEtiketSayisi,nBasilacakTekliEtiketSayisi,[sUrunEtiketAciklama],[dtOlusturma],[tmOlusturma],sSiparisReferansNo,bMailIletimi)
values
(@pIsemri,@ldIsEmriMiktari,@StokKodu_Rowid,@StokAdi,@ldPaketeGirecekAdet,1,@ldCokluEtiketSayisi,@ldTekliEtiketSayisi,@EtiketAciklamasi,CAST(GETDATE() as date),replace(Convert (varchar(8),GetDate(), 108),':',''),@sSipRefereransNo,'H');
...
BEGIN
		...
		insert into [Gusto].[Bpm_IsEmriEtiketSatirABD] ([pIsEmriEtiketBaslik],[ldMiktar],[pStokKodu],[sStokKodu],[sStokAdi],sUreticiKodu,pIsemri,sPaketlemeTuru,bTekliCokluPaket,sSiparisReferansNo) values (@Bpm_IsEmriEtiketBaslik_Rowid,@KalanMiktar,@StokKodu_Rowid,@StokKodu,@StokAdi,@sUreticiKodu,@pIsemri,'Çoklu Paket',2,@sSipRefereransNo);
   
		set @KalanMiktar=@ldIsEmriMiktari-(select sum(ldMiktar) from [Gusto].[Bpm_IsEmriEtiketSatirABD] where [pIsEmriEtiketBaslik]=@Bpm_IsEmriEtiketBaslik_Rowid)
...
BEGIN
		...
		insert into [Gusto].[Bpm_IsEmriEtiketSatirABD] ([pIsEmriEtiketBaslik],[ldMiktar],[pStokKodu],[sStokKodu],[sStokAdi],sUreticiKodu,pIsemri,sPaketlemeTuru,bTekliCokluPaket,
		sSiparisReferansNo) 
		--values 
		--(@Bpm_IsEmriEtiketBaslik_Rowid,1,@StokKodu_Rowid,@StokKodu,@StokAdi,@sUreticiKodu,@pIsemri,@EtiketAciklamasi,1,@sSipRefereransNo);
