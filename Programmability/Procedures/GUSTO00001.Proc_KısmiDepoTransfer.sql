...
GO
CREATE PROCEDURE [GUSTO00001].[Proc_KısmiDepoTransfer]
...
--declare @pOlusturanKullanici int;
--declare @pFaaliyet int;
--declare @pIsemri int;
--declare @PR_ID int ;


--set @pOlusturanKullanici=1;
--set @pFaaliyet=14640;
--set @pIsemri=10703;
--set @PR_ID=1;
...
values (GETDATE(),@sSeri,@nNo,@pGirisDepo,@pCikisDepo,@sAciklama,'G',@pOlusturanKullanici,cast(GETDATE() as date),@tmOlusturma);
...
DECLARE Kısmi_Depo_Transfer CURSOR FOR

select StokKodu_rowid,isnull(case when iem.ldKullanimAdediBirim1>=dsma.ldBirinciBirim then dsma.ldBirinciBirim
when iem.ldKullanimAdediBirim1<dsma.ldBirinciBirim then iem.ldKullanimAdediBirim1 end,0)
from Gusto00001.IsEmriMalzemeler iem WITH (NOLOCK)
inner join Gusto00001.StokKodu s WITH (NOLOCK) on s.StokKodu_rowid=iem.pStokKodu
inner join Gusto00001.IsEmri i  WITH (NOLOCK) on i.IsEmri_rowid=iem.pIsemri
left join GUSTO00001.DepoStokMiktar dsm WITH (NOLOCK) on dsm.pStokKodu=s.StokKodu_rowid and dsm.pDepoKodu=2
left join GUSTO00001.DepoStokMiktar dsma WITH (NOLOCK) on dsma.pStokKodu=s.StokKodu_rowid and dsma.pDepoKodu=1
where  exists  
(select * from Gusto.EPMItemValue_Multi WITH (NOLOCK) where PEKBILGITURU=2718 and iem.IsEmriMalzemeler_rowid=PITEM and PFAALIYET=@pFaaliyet)

OPEN Kısmi_Depo_Transfer
FETCH NEXT FROM Kısmi_Depo_Transfer INTO @pStokKodu,@ldMiktar
...
CLOSE Kısmi_Depo_Transfer

DEALLOCATE Kısmi_Depo_Transfer

update GUSTO00001.Seri set nNo=@nNo where sSeri='DT' and nHareketTuru=4000;

if ((select COUNT(*) from Gusto.BPMCG_OlusanDepoTransferleri WITH (NOLOCK) where bTransferTuru=1094 and
...
((select ROWID from Gusto.BPMFT_DEPOTRANSFERI where PFAALIYET=@pFaaliyet),@DepoHareketBaslik_Rowid,1094);
