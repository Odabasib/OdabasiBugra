...
GO
CREATE PROCEDURE [GUSTO00001].[Proc_TopluDepoTransfer]
...
set @tmOlusturma=(select CONVERT(int,CONCAT(DATEPART(HOUR, GETDATE()),DATEPART(MINUTE, GETDATE()),DATEPART(SECOND, GETDATE()))));
set @sSeri='DT';
...
values (GETDATE(),@sSeri,@nNo,@pGirisDepo,@pCikisDepo,@sAciklama,'G',@pOlusturanKullanici,GETDATE(),@tmOlusturma);
...
DECLARE Toplu_Depo_Transfer CURSOR FOR

select StokKodu_rowid,case when iem.ldKullanimAdediBirim1>=dsma.ldBirinciBirim then dsma.ldBirinciBirim
when iem.ldKullanimAdediBirim1<dsma.ldBirinciBirim then iem.ldKullanimAdediBirim1 end
from Gusto00001.IsEmriMalzemeler iem
inner join Gusto00001.StokKodu s on s.StokKodu_rowid=iem.pStokKodu
inner join Gusto00001.IsEmri i on i.IsEmri_rowid=iem.pIsemri
left join GUSTO00001.DepoStokMiktar dsm on dsm.pStokKodu=s.StokKodu_rowid and dsm.pDepoKodu=2
left join GUSTO00001.DepoStokMiktar dsma on dsma.pStokKodu=s.StokKodu_rowid and dsma.pDepoKodu=1
where i.IsEmri_rowid=@pIsemri and iem.pStokKodu not in 
(select pStokKodu from Gusto00001.Urunler) and dsma.ldBirinciBirim>0

OPEN Toplu_Depo_Transfer
FETCH NEXT FROM Toplu_Depo_Transfer INTO @pStokKodu,@ldMiktar
...
(@pCikisDepo,GETDATE(),@sSeri,@nNo,'1',1,@pStokKodu,0,'1',@ldMiktar,@DepoHareketBaslik_Rowid,@pOlusturanKullanici,'');
...
insert into GUSTO00001.DepoHareket (pDepo,dtTarih,sSeri,lNo,cGirisCikis,nSira,pStokKodu,pIzlemeKodu,cKacinciBirim,ldMiktar1,pBaslik,pOlusturanKullanici,sAciklama) values 
(@pGirisDepo,GETDATE(),@sSeri,@nNo,'0',1,@pStokKodu,0,'1',@ldMiktar,@DepoHareketBaslik_Rowid,@pOlusturanKullanici,'');
...
CLOSE Toplu_Depo_Transfer

DEALLOCATE Toplu_Depo_Transfer

update GUSTO00001.Seri set nNo=@nNo where sSeri='DT' and nHareketTuru=4000;

if ((select COUNT(*) from [Gusto].[DepoTransferLog] where pFaaliyet=@pFaaliyet and bTopluDepoTransfer_Rowid in 
(select DepoHareketBaslik_rowid from GUSTO00001.DepoHareketBaslik))=0) begin

insert into [Gusto].[DepoTransferLog] (pFaaliyet,bTopluDepoTransfer_Rowid) values (@pFaaliyet,@DepoHareketBaslik_Rowid);
insert into Gusto.BPMCG_OlusanDepoTransferleri (PPARENT,pOlusanTransfer,bTransferTuru) values 
((select ROWID from Gusto.BPMFT_DEPOTRANSFERI where PFAALIYET=@pFaaliyet),@DepoHareketBaslik_Rowid,1096);
end

else begin

update [Gusto].[DepoTransferLog] set bTopluDepoTransfer_Rowid=@DepoHareketBaslik_Rowid where pFaaliyet=@pFaaliyet;
end
