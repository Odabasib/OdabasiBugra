...
GO
CREATE PROCEDURE [GUSTO00001].[SayimFarkiCikisOlusturma]
@pDepo int
...
SET NOCOUNT ON;
	...
	begin
		...
		--(select * 
		--from GUSTO00001.StokHareketBaslik where StokHareketBaslik_rowid=23189);
		...
		values
		(@StokhareketBaslik_Rowid,@sSeri,@nNo,'S',@pStokKodu,@pDepomaliyetKodu,'1',@nHareketTuru,'1',
		...
		values
		(@pDepo,GETDATE(),@sSeri,@nNo,'1',1,@nHareketTuru,@pStokKodu,'1',@ldBirinciBirim,@ldIkinciBirim,@ldUcuncuBirim,1,@dtOlusturma,@tmOlusturma,@sAciklama)
		set @pDepoHareket =SCOPE_IDENTITY()

		UPDATE Gusto00001.StokHareket 
		set pHareketBaslik=@StokhareketBaslik_Rowid,sSeri=@sSeri,lNo=@nNo,cStokHizmetDemirbas='S',pStokKodu=@pStokKodu,pDepoMaliyetKodu=@pDepomaliyetKodu,cGirisCikis='1',
