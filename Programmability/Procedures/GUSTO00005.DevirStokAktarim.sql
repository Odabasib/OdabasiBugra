...
GO
CREATE PROCEDURE [GUSTO00005].[DevirStokAktarim]
...
SET NOCOUNT ON;
	...
	set @BirimldFiyat = (select Cast(REPLACE(@sBirimldFiyat,',','.') as decimal(22,6) ) );

	SET @pStokKodu = (SELECT StokKodu_rowid FROM GUSTO00005.StokKodu where sKod=@sStokKodu);
	SET @pDepoKodu = (SELECT DepoKodu_rowid FROM GUSTO00005.DepoKodu where sKod=@sDepoKodu);
	SET @pDepoMaliyetKodu = (SELECT pDepoMaliyetKodu FROM GUSTO00005.DepoKodu where sKod=@sDepoKodu)

	SET @ldMiktar2 = ISNULL((SELECT CASE WHEN cIkinciBirimCarpanBolen='C' THEN @ldMiktar/ldIkinciBirimKatsayisi ELSE @ldMiktar*ldIkinciBirimKatsayisi END 
						FROM GUSTO00005.StokDetay WHERE pStokKodu=@pStokKodu
						AND sIkinciBirimAdi<>'' AND ldIkinciBirimKatsayisi>0),0);
	
	SET @ldMiktar3 = ISNULL((SELECT CASE WHEN cUcuncuBirimCarpanBolen='C' THEN @ldMiktar/ldUcuncuBirimKatsayisi ELSE @ldMiktar*ldUcuncuBirimKatsayisi END 
						FROM GUSTO00005.StokDetay WHERE pStokKodu=@pStokKodu
	...
	SET @StokhareketBaslik_Rowid = ISNULL((SELECT StokhareketBaslik_Rowid FROM GUSTO00005.StokHareketBaslik
	...
	BEGIN
		...
		INSERT INTO GUSTO00005.IslemTablosu 
		...
		INSERT INTO GUSTO00005.StokHareketBaslik
		...
		INSERT INTO GUSTO00005.FaturaKDV (cBelgeTuru,pBaslik) VALUES ('SH',@StokhareketBaslik_Rowid);

		UPDATE GUSTO00005.Seri SET nNo=(SELECT MAX(lNo)+1 FROM GUSTO00005.StokHareketBaslik WHERE nHareketTuru=@nHareketTuru AND sSeri=@sSeri)
		WHERE nHareketTuru = @nHareketTuru and sSeri=@sSeri;
	END

	DECLARE @StokHareket_Rowid int;
	INSERT INTO GUSTO00005.StokHareket 
	...
	INSERT INTO GUSTO00005.DepoHareket 
	...
	INSERT INTO GUSTO00005.StokYuruyenMaliyet 
	...
	DELETE FROM GUSTO00005.StokYuruyenMaliyet WHERE ((pStokHareket=@StokHareket_Rowid AND pIzlemeKodu=0) AND pDepoMaliyetKodu=@pDepoMaliyetKodu);
	...
	DELETE FROM GUSTO00005.DepoStokMiktar WHERE pStokKodu=@pStokKodu and pDepoKodu=@pDepoKodu
