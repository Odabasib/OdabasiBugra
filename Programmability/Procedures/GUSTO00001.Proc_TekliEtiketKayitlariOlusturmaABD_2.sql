...
GO
CREATE PROCEDURE [GUSTO00001].[Proc_TekliEtiketKayitlariOlusturmaABD_2] 
...
select ise.IsEmri_rowid,ise.ldPlanlananIsEmriMiktari [İş Emri Adedi],s.StokKodu_rowid [StokKodu_Rowid],s.sAd [Stok Adı],0 [Pakete Girecek Olan Ürün Sayısı],1  [bTekliCokluPaket],'Sadece Tekli Paketlenecek' [Paketleme Açıklaması],s.sKod [StokKodu],asb.pCari,
...
where not exists (select * from GUSTO00001.stokEkAlanSayi where pEkAlanTanimi=40 and pOwnerID=s.StokKodu_rowid)				   
...
where op.pOperasyonKodu=21 and opr.pIsemri=ise.IsEmri_rowid and CAST(opr.dtIslemBaslamaTarihi as date)=CAST(GETDATE() as date)) and not exists (select * from [Gusto].[Bpm_IsEmriEtiketBaslikABD] where pIsemri=ise.IsEmri_rowid)
and asb.pCari =900
...
inner join GUSTO00001.StokKodu s on s.StokKodu_rowid=iu.pStokKodu
inner join (select * from GUSTO00001.stokEkAlanSayi where pEkAlanTanimi=41 and ldSayi=1)tp on tp.pOwnerID=s.StokKodu_rowid		--Tekli Paket/Çoklu Paket Ek Alan Tanımı
inner join (select * from GUSTO00001.stokEkAlanSayi where pEkAlanTanimi=40 and ldSayi=0)tp1 on tp1.pOwnerID=s.StokKodu_rowid	--Çoklu Pakete Girecek Adet Ek Alan Tanımı
...
FETCH NEXT FROM TekliPaketBaslikBilgileri INTO @pIsemri,@ldIsEmriMiktari,@StokKodu_Rowid,@StokAdi,@ldPaketeGirecekAdet,@bTekliCokluPaket,@EtiketAciklamasi,@StokKodu,@pCari,@sUreticiKodu,@ldTekliEtiketSayisi,@ldCokluEtiketSayisi,@sSipRefereransNo
WHILE @@FETCH_STATUS =0

BEGIN

insert into [Gusto].[Bpm_IsEmriEtiketBaslikABD] ([pIsemri],[ldIsEmriMiktari],[pStokKodu],[sStokAdi],[ldPaketeGirecekMiktar],[bTekliCokluPaket],nBasilacakTekliEtiketSayisi,nBasilacakCokluEtiketSayisi,[sUrunEtiketAciklama],[dtOlusturma],[tmOlusturma],sSiparisReferansNo,bMailIletimi)
values
(@pIsemri,@ldIsEmriMiktari,@StokKodu_Rowid,@StokAdi,@ldPaketeGirecekAdet,1,@ldTekliEtiketSayisi,@ldCokluEtiketSayisi,@EtiketAciklamasi,CAST(GETDATE() as date),replace(Convert (varchar(8),GetDate(), 108),':',''),@sSipRefereransNo,'H');
...
BEGIN
		...
		insert into [Gusto].[Bpm_IsEmriEtiketSatirABD] ([pIsEmriEtiketBaslik],[ldMiktar],[pStokKodu],[sStokKodu],[sStokAdi],sUreticiKodu,pIsemri,sPaketlemeTuru,bTekliCokluPaket,sSiparisReferansNo) values (@Bpm_IsEmriEtiketBaslik_Rowid,1,@StokKodu_Rowid,@StokKodu,@StokAdi,@sUreticiKodu,@pIsemri,@EtiketAciklamasi,1,@sSipRefereransNo);
