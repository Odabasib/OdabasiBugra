...
GO
CREATE PROCEDURE [GUSTO00001].[Proc_CokluPaketEtiketKayitlariOlusturmaRenzel] 
...
select ise.IsEmri_rowid,ise.ldPlanlananIsEmriMiktari [İş Emri Adedi],s.StokKodu_rowid [StokKodu_Rowid],case when len(rz.sMetin)!=0 then rz.sMetin else s.sAd end  [Renzel Stok Adı],tpc.ldSayi [Pakete Girecek Olan Ürün Sayısı],2  [bTekliCokluPaket],
...
FETCH NEXT FROM CokluPaketBaslikBilgileri INTO @pIsemri,@ldIsEmriMiktari,@StokKodu_Rowid,@StokAdi,@ldPaketeGirecekAdet,@bTekliCokluPaket,@ldCokluEtiketSayisi,@EtiketAciklamasi,@StokKodu,@pCari,@sUreticiKodu,@ldTekliEtiketSayisi,@sSipRefereransNo,@sYilinKacinciHaftasi
...
values
(@pIsemri,@ldIsEmriMiktari,@StokKodu_Rowid,@StokAdi,@ldPaketeGirecekAdet,2,@ldTekliEtiketSayisi,@ldCokluEtiketSayisi,@EtiketAciklamasi,CAST(GETDATE() as date),replace(Convert (varchar(8),GetDate(), 108),':',''),@sSipRefereransNo,'H',@sYilinKacinciHaftasi);
