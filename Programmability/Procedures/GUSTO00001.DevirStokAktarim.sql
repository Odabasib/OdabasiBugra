...
GO
CREATE PROCEDURE [GUSTO00001].[DevirStokAktarim]
...
SET NOCOUNT ON;
	...
	SET @pDepoKodu = (SELECT DepoKodu_rowid FROM GUSTO00001.DepoKodu where sKod=@sDepoKodu);
	SET @pDepoMaliyetKodu = (SELECT pDepoMaliyetKodu FROM GUSTO00001.DepoKodu where sKod=@sDepoKodu)
	...
	SET @ldMiktar2 = ISNULL((SELECT CASE WHEN cIkinciBirimCarpanBolen='C' THEN @ldMiktar/ldIkinciBirimKatsayisi ELSE @ldMiktar*ldIkinciBirimKatsayisi END 
						FROM GUSTO00001.StokDetay WHERE pStokKodu=@pStokKodu
	...
	SET @StokhareketBaslik_Rowid = ISNULL((SELECT StokhareketBaslik_Rowid FROM GUSTO00001.StokHareketBaslik
	...
	BEGIN
		...
		INSERT INTO GUSTO00001.IslemTablosu 
		...
		INSERT INTO GUSTO00001.StokHareketBaslik
		...
		INSERT INTO GUSTO00001.FaturaKDV (cBelgeTuru,pBaslik) VALUES ('SH',@StokhareketBaslik_Rowid);

		UPDATE GUSTO00001.Seri SET nNo=(SELECT MAX(lNo)+1 FROM GUSTO00001.StokHareketBaslik WHERE nHareketTuru=@nHareketTuru AND sSeri=@sSeri)
	...
	INSERT INTO GUSTO00001.DepoHareket 
	...
	INSERT INTO GUSTO00001.StokYuruyenMaliyet 
	...
	DELETE FROM GUSTO00001.StokYuruyenMaliyet WHERE ((pStokHareket=@StokHareket_Rowid AND pIzlemeKodu=0) AND pDepoMaliyetKodu=@pDepoMaliyetKodu);
	...
	DELETE FROM GUSTO00001.DepoStokMiktar WHERE pStokKodu=@pStokKodu and pDepoKodu=@pDepoKodu
