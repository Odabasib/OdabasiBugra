CREATE TABLE [GUSTO00001].[IsEmriOperasyon] (
...
CREATE INDEX [pIsemri]
  ON [GUSTO00001].[IsEmriOperasyon] ([pIsemri])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [pOperasyonKodu]
  ON [GUSTO00001].[IsEmriOperasyon] ([pOperasyonKodu])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO
...
GO
CREATE TRIGGER [ADTRDIsEmriOperasyon] ON [GUSTO00001].[IsEmriOperasyon] FOR DELETE AS DECLARE @pGustoUser int;SET @pGustoUser = isnull((SELECT top 1 pKullanici FROM Gusto.Kilit Where sBilgisayarAdi=HOST_NAME() and pFirma=1),0); INSERT INTO Gusto00001.ADTIsEmriOperasyon(sTableName,pTableRowId,cIslem,sDbUser,pGustoUser,sHostName,dtOlusturma,tmOlusturma) select 'IsEmriOperasyon',IsEmriOperasyon_rowid,'D',USER_NAME(),@pGustoUser,HOST_NAME(),GetDate(), DATEPART(hh,GETDATE())*10000 + DATEPART(mi,GETDATE())*100 + DATEPART(ss,GETDATE()) FROM DELETED;
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS OFF
GO
CREATE TRIGGER [ADTRUIsEmriOperasyon] ON [GUSTO00001].[IsEmriOperasyon] FOR UPDATE AS DECLARE @pGustoUser int;SET @pGustoUser = isnull((SELECT top 1 pKullanici FROM Gusto.Kilit Where sBilgisayarAdi=HOST_NAME() and pFirma=1),0); INSERT INTO Gusto00001.ADTIsEmriOperasyon(sTableName,pTableRowId,cIslem,sDbUser,pGustoUser,sHostName,dtOlusturma,tmOlusturma) select 'IsEmriOperasyon',IsEmriOperasyon_rowid,'U',USER_NAME(),@pGustoUser,HOST_NAME(),GetDate(), DATEPART(hh,GETDATE())*10000 + DATEPART(mi,GETDATE())*100 + DATEPART(ss,GETDATE()) FROM INSERTED;
GO