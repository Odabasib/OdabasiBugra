CREATE TABLE [GUSTO00001].[DepoHareket] (
  [DepoHareket_rowid] [int] IDENTITY,
  ...
  PRIMARY KEY CLUSTERED ([DepoHareket_rowid]) WITH (FILLFACTOR = 90)
)
ON [PRIMARY]
GO

CREATE INDEX [Baslik]
  ON [GUSTO00001].[DepoHareket] ([pBaslik])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [Depo]
  ON [GUSTO00001].[DepoHareket] ([pDepo])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [INDX_DEPOH_001]
  ON [GUSTO00001].[DepoHareket] ([nHareketTuru], [pDepo], [pIzlemeKodu])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [SeriNo]
  ON [GUSTO00001].[DepoHareket] ([lNo])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [Sira]
  ON [GUSTO00001].[DepoHareket] ([nSira])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [Stok]
  ON [GUSTO00001].[DepoHareket] ([pStokKodu], [pIzlemeKodu])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [Tarih]
  ON [GUSTO00001].[DepoHareket] ([dtTarih])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO