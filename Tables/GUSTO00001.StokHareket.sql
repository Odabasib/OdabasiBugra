CREATE TABLE [GUSTO00001].[StokHareket] (
...
CREATE INDEX [BagliSatir]
  ON [GUSTO00001].[StokHareket] ([pBagliOlduguSatir])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [CariKodu]
  ON [GUSTO00001].[StokHareket] ([pCari])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [DepoMaliyet]
  ON [GUSTO00001].[StokHareket] ([pDepoMaliyetKodu])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [FiyatListesi]
  ON [GUSTO00001].[StokHareket] ([pFiyatListesi])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [HareketBaslik]
  ON [GUSTO00001].[StokHareket] ([pHareketBaslik])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [INDX_STOKH_001]
  ON [GUSTO00001].[StokHareket] ([nHareketTuru], [pDepo], [pIzlemeKodu])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [IslemTablosu]
  ON [GUSTO00001].[StokHareket] ([pIslemID])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [Sira]
  ON [GUSTO00001].[StokHareket] ([nSira])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [StokDepo]
  ON [GUSTO00001].[StokHareket] ([pStokKodu], [pDepo])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [StokIzleme]
  ON [GUSTO00001].[StokHareket] ([pStokKodu], [pIzlemeKodu])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [StokKodu]
  ON [GUSTO00001].[StokHareket] ([pStokKodu])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO

CREATE INDEX [Tarih]
  ON [GUSTO00001].[StokHareket] ([dtTarih])
  WITH (FILLFACTOR = 90)
  ON [PRIMARY]
GO
...
-- Description:	Silinen  Alım İade kaydının Gusto.GirisKalite Harici tablosunda bulunan kaydını güncellemesi için create edilmiştir.
-- =============================================
CREATE TRIGGER [Giriskalite]
   ON  [GUSTO00001].[StokHareket]
   AFTER DELETE
AS 
BEGIN
...
declare @sBool varchar(4);


select @Stokhareket_Rowid=StokHareket_rowid,@nHareketTuru=nHareketTuru from deleted;

set @sBool=(select IIF(((select COUNT(*) from Gusto.GirisKalite where pIadeStokHareket=@Stokhareket_Rowid)>0),'TRUE','ELSE'))

if(@nHareketTuru=213 and @sBool='TRUE') begin

delete from  Gusto.GirisKalite  where pIadeStokHareket=@Stokhareket_Rowid;
